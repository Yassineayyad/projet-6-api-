...
// Fs = file system donne accès aux fonctions qui nous permettent de modifier le système de fichiers, y compris aux fonctions permettant de supprimer les fichiers.
const fs = require("fs");

exports.createSauce = (req, res, next) => {
  const sauceObject = JSON.parse(req.body.sauce); // on parse les données envoyés pr pouvoir les manipuler
  ...
  const sauce = new Sauce({
    ...
    likes: 0,
    dislikes: 0,
        // on rajoute les like et dislike vide 
    usersLiked: [],
    usersDisliked: []
  });
  sauce
    .save()
    .then(() => res.status(201).json({ message: "Sauce crée avec succes!" }))
    .catch((error) => res.status(400).json({ error }));
};

exports.modifySauce = (req, res, next) => {
  if (req.file) {
    //supprimé l'image si l'utilisater modifie son image
